...
# setting fea embedding:
FEA_FEED_LIST += [f"embed{i}" for i in range(512)]
...
if __name__ == "__main__":
    ...
    for action in ACTION_LIST:
        ...
        # adding val data:
        val = pd.read_csv(ROOT_PATH + f'/val_data.csv')[USE_FEAT]
        ...
        # concat the train val test data
        data = pd.concat((train, val, test)).reset_index(drop=True)

        dense_features = ['videoplayseconds']

        # adding embedding feature
        embed_features = [f"embed{i}" for i in range(512)]

        sparse_features = [i for i in USE_FEAT if i not in dense_features+embed_features and i not in target]
        ...
        fixlen_feature_columns = [SparseFeat(feat, data[feat].nunique()) for feat in sparse_features] + [DenseFeat(feat, 1, ) for feat in dense_features] + [DenseFeat(feat, 1, ) for feat in embed_features]
        ...
        # create train, val, tes data
        train, val, test = data.iloc[:train.shape[0]].reset_index(drop=True), data.iloc[train.shape[0]: train.shape[0] + val.shape[0]].reset_index(drop=True), data.iloc[train.shape[0] + val.shape[0]:].reset_index(drop=True)

        train_model_input = {name: train[name] for name in feature_names}
        test_model_input = {name: test[name] for name in feature_names}

        # adding val model input
        ...
        # new model adding validation_data
        history = model.fit(train_model_input, train[target].values, batch_size=512, epochs=5, verbose=1,
                            validation_data=(val_model_input, val_model_input[target].values))
