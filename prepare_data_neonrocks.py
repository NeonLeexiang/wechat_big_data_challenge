...
'''
    然后需要改的地方有：
    1.分出来验证集和测试集，把前12天作为训练集，第13天作为验证集，第14天作为测试集；
    2.把评分改为和官方baseline评分机制一样；
    3.将feed_info特征和feed_embedding特征加进去
'''
...
def prepare_data():
    ...
    print('step[2] ---> merge user_action and feed_info')
    train = pd.merge(user_action_df, feed_info_df[FEA_FEED_LIST], on='feedid', how='left')

    # adding embedding
    print('step[3] ---> merge train data and feed_embedding')
    train = pd.merge(train, feed_embed, on='feedid', how='left')

    # processing embedding
    print('step[4] ---> processing embedding data')
    train = process_embed(train)

    # split it into train, val, test
    print('step[5] ---> split it into train, val, test data and output the data')
    print('dir is ./data/wechat_algo_data1')
    val = train[train['date_'] == 13]
    test = train[train['date_'] == 14]
    train = train[train['date_'] < 13]

    val["videoplayseconds"] = np.log(val["videoplayseconds"] + 1.0)
    test["videoplayseconds"] = np.log(test["videoplayseconds"] + 1.0)
    val.to_csv(ROOT_PATH + f'/val_data.csv', index=False)
    test.to_csv(ROOT_PATH + f'/test_data.csv', index=False)

    # test = pd.merge(test, feed_info_df[FEA_FEED_LIST], on='feedid', how='left')

    # adding embedding
    # test = pd.merge(test, feed_embed['feed_embedding'][0], on='feedid', how='left')

    # test["videoplayseconds"] = np.log(test["videoplayseconds"] + 1.0)
    # test.to_csv(ROOT_PATH + f'/test_data.csv', index=False)

    print('step[6] ---> processing action training data')
